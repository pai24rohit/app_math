...
def test_define_dense_model_with_hidden_layer():
    ...
    assert model.input_shape == (None, 43), " input_shape is not correct"
    assert model.layers[0].output.shape == (None, 11), " hidden layer output_shape is not correct"
    assert model.layers[1].output.shape == (None, 1), " output layer output_shape is not correct"
